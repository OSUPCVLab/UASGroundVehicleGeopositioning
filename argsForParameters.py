...
def parseArgs(verbose = False):
    ...
    # parser.add_argument('--cacheDetDir', type=str, default='sampleData/cachedDetections', help='where to cache detections for each frame')
    # parser.add_argument('--cacheTrackDir', type=str, default='sampleData/cachedTracking', help='where to cache tracking for each frame')
    # parser.add_argument('--cache_mkptsDir', type=str, default='sampleData/cachedmkpts_', help='where to cache keypoints for each frame')
    parser.add_argument('--filterCars', default = True, type = evalBool, help='whether or not to filter cars')
    parser.add_argument('--filterRoads', default = True, type = evalBool, help='whether or not to filter roads')
    parser.add_argument('--filterBuildings', default = True, type = evalBool, help='whether or not to filter buildings')
    parser.add_argument('--SuperGlue', default = True, type = evalBool, help='whether or not to use SuperGlue features')
    parser.add_argument('--LoFTR', default = False, type = evalBool, help='whether or not to use LoFTR features')
    parser.add_argument('--homography', default = True, type = evalBool, help='True for homography, False for affine2D')
